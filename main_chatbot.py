...
class MiniAgenticAISystem:
    def __init__(self):
        ...
        # Sample data for demonstration (will be used only if no JSON is provided in query)
        self.sample_data = [
            {
                "campaign_id": "CAMP_A",
    ...
    def extract_json_from_query(self, query: str) -> Any:
        ...
        # Look for JSON patterns in the query
        json_patterns = [
            ...
            if matches:
                for match in matches:
                    try:
                        ...
                        # If it fails, try to clean up the string and parse again
                        try:
                            # Remove any potential trailing commas or other issues
    ...
    def route_query(self, query: str) -> str:
        ...
        # First check if the query contains JSON data
    ...
    def reporting_tool(self, query: str) -> Dict[str, Any]:
        """Format JSON data based on user request into tables or structured summaries"""
        # First try to extract JSON from the query
        json_data = self.extract_json_from_query(query)
        
        # If no JSON found in query, use sample data and notify user
        ...
        system_message = """You are a reporting assistant specialized in formatting JSON data into clear, readable tables.
        The user will provide a query about how to format or present JSON data. 
        ...
        Structure your response as a clear table with appropriate headers."""
        
        # Format the data
        formatted_output = self.get_llm_response(
            f"User query: {query}\n\nJSON data to format: {json.dumps(json_data, indent=2)}",
            system_message
        )
